...
from data.uavdt_dataset import UAVDTDataset
...
def get_dataset(
    ...
    dataset = UAVDTDataset(root_dir=root_dir, split=split, transform=transform)
...
def transformation(split: str):
    if split == "train":
        return transforms.Compose(
            [
                transforms.Resize((540, 1024)),  # fixed size (height, width)
                transforms.ToTensor(),
            ]
        )
