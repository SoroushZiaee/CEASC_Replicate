           ...
           for chunk in response.iter_content(chunk_size=8192):
               if chunk:
                   ...
                   if total_size > 0:
                       ...
                       print(f"\rProgress: {percent:.1f}% ({downloaded/1_000_000:.1f}MB / {total_size/1_000_000:.1f}MB)", end="")
...
def download(urls, dir, threads=4, extract=True):
   ...
   with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
       future_to_url = {executor.submit(download_file, url, dir_path): url for url in urls}
       downloaded_files = []
