...
class UAVDTDataset(Dataset):
    def __init__(self, root_dir, split: str = "train", transform=None):
        ...
        self.categories = {0: "car", 1: "truck", 2: "bus"}

        meta_path = os.path.join(
            self.root_dir, "UAV-benchmark-M", f"{split}set_meta.csv"
        )
    ...
    def parse_annotation(self, movie_name, img_idx):
        ...
        anno_path = os.path.join(
            self.root_dir, "UAV-benchmark-MOTD_v1.0", "GT", f"{movie_name}_gt_whole.txt"
        )
        with open(anno_path, "r") as anno_file:
            annos = anno_file.readlines()
            our_img_annos = [
                a for a in annos if int(a.split(",")[0]) == img_idx
            ]  # get all the annotation lines that correspond to our image
            for anno in our_img_annos:
                items = [
                    int(i) for i in anno.split(",")
                ]  # get all the individual annotations
                # print(f"{items = }")
                [
                    frame_index,
                    target_id,
                ...
                labels.append(
                    object_category - 1
                )  # the categories are coded as one less than the true numerical value in the dataset
